    ...
    for (int i = 4; i < parsedAsmInstructions.size(); i++)
    {
        ...
        for(int j = parsedAsmInstructions[i].size() - 1; j >= 0; j--)
        {
            ...
            if(InstructionEnumUtils::isStringInInstructionsEnum(instruction)) 
            {
                ...
                {      
                    // TODO: a lot of logic is duplicated (just hard assigning values) and cand ef be split out
                    ...
                    case Instructions::mul:
                        // add opcode changes depending on what is being added :( 
                        if (intruction_type_map.size() == 1) // mul takes one operand so map should always only be instr op
                        {
                            // The mul operation also has a mode field which changes depending on whats getting added 
                            // 00 - memory, no displacement 
                            // 01 - mem, 8bit signed displacement
                            // 10 - mem, 32bit signed displacement
                            // 11 - reg direct mode i.e. both operands are registers 

                            // For command mul reg reg 
                            if(std::get<0>(intruction_type_map[0]) == instructionType::register_name)
                            {
                                const uint8_t mul_two_reg = 0xF7; //opcode
                                std::string mul_two_reg_string = convertToHexString(mul_two_reg);

                                const uint8_t mul_mod_value = 0x03; // register direct mode meaning utilazing to reg instead of like reg and literal 
                                 // Some instructions have extension as part of there ModR/M byte shennangins 
                                 // [mod | reg | r/m]
                                 // mod -> shown above 
                                 // reg -> register or op code extension 
                                 // r/m -> register or memory 
                                const uint8_t mul_op_code_extension = 0x04; // TODO: this should just be an enum 
                                std::string reg1_string =  convertToHexString(std::get<1>(intruction_type_map[0]));
                                // std::string reg2_string =  convertToHexString(std::get<1>(intruction_type_map[1]));
                                // the ModR/M byte is for insturctions and specifies the operanrds for an instruction 
                                int mod_r_m_reg = ModR_M_Set(mul_mod_value, mul_op_code_extension, std::get<1>(intruction_type_map[0]));
                                std::string mod_r_m_reg_string = convertToHexString(mod_r_m_reg);

                                std::vector<std::string> line;
                                line.push_back(rexByteString);
                                line.push_back(mul_two_reg_string);
                                line.push_back(mod_r_m_reg_string);
                                bytestring.push_back(line);

                                std::cout << "This is the add instruction as Bytes: " << rexByteString << mul_two_reg_string << mod_r_m_reg_string << std::endl;
